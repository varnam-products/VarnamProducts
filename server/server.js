...
import internationalOrderRoutes from './routes/internationalOrderRoutes.js';
...
const allowedOrigins = [process.env.CLIENT_URL, 'http://localhost:3000', 'https://varnam-products-frontend.vercel.app', 'https://varnam-products-umber.vercel.app'].filter(Boolean);
app.use(
  cors({
...
const internationalOrderLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 5,
  message: { success: false, message: 'Too many requests submitted. Please try again later.' },
  standardHeaders: true,
  legacyHeaders: false,
});
...
app.use('/api/international-orders/submit', internationalOrderLimiter);
app.use('/api/contact/submit', contactMessageLimiter);
...
app.use('/api/international-orders', internationalOrderRoutes);
