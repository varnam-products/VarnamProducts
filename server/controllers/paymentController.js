...
export const createCashfreeOrder = async (req, res) => {
  try {
    ...
    for (const item of orderItems) {
      ...
      if (!item.variantId) return res.status(400).json({ success: false, message: 'Each order item must specify a variant' });
      ...
      // Stock is still one shared pool per product, but price is now per-variant —
      // resolve which variant was actually selected and price/snapshot off of that,
      // never trusting a price the client may have sent.
      const variant = dbProduct.variants.id(item.variantId);
      if (!variant) return res.status(400).json({ success: false, message: 'Product variant not found' });
