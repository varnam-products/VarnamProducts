...
export const createCashfreeOrder = async (req, res) => {
  try {
    const { orderItems, customerName, customerEmail, customerPhone, shippingAddress, idempotencyKey, couponCode } = req.body;
    ...
    // Include variantId in the identity used for draft matching — otherwise the same
    // product bought in two different variants (e.g. 500ml vs 1L) could incorrectly
    // match a stale draft from the other variant.
    const incomingProductIds = incomingSorted.map(i => `${i.product}:${i.variantId}`);
    ...
    if (draftMatchQuery) {
      ...
      if (existingActiveDraft) {
        ...
        const draftProductIds = draftSorted.map(i => `${i.product}:${i.variantId}`);
        const draftQuantities = draftSorted.map(i => i.quantity);
        const incomingCouponCode = couponCode ? String(couponCode).trim().toUpperCase() : null;
        const draftCouponCode = existingActiveDraft.appliedCoupon?.code || null;
        const itemsMatch = JSON.stringify(incomingProductIds) === JSON.stringify(draftProductIds) &&
          ...
          incomingCouponCode === draftCouponCode;
    ...
    let verifiedOrderItems, calculatedSubtotal, trustedShippingFee, appliedCoupon, trustedTotalAmount;
    try {
      ({
        finalOrderItems: verifiedOrderItems,
        subtotal: calculatedSubtotal,
        shippingFee: trustedShippingFee,
        appliedCoupon,
        totalPrice: trustedTotalAmount,
      } = await calculateOrderPricing({ orderItems, settings, couponCode }));
    } catch (pricingErr) {
      const clientSafe = ['Invalid quantity', 'Product not found', 'Product variant not found', 'Insufficient stock', 'Invalid or expired coupon code'].includes(pricingErr.message);
      return res.status(clientSafe ? 400 : 500).json({ success: false, message: clientSafe ? pricingErr.message : 'Something went wrong' });
    }
...
/**
 * Voids a still-pending Cashfree draft order — used when a customer abandons an
 * online-payment attempt and switches to a different payment method (e.g. COD) for
 * the same cart. Without this, the abandoned draft just sits as "Pending Payment"
 * for up to 24h (until its TTL expiry) even though the customer already completed
 * a real order a different way.
 *
 * Deliberately reuses handlePaymentFailed — voiding a draft and a genuine payment
 * failure land the order in the exact same place (paymentStatus: FAILED,
 * orderStatus: CANCELLED), so there's no reason to duplicate that logic here.
 *
 * Safe by construction: only ever touches an order that is still PENDING. If a
 * webhook happened to land in the meantime and the "abandoned" payment actually
 * succeeded, this is a no-op — we never overwrite a resolved order.
 */
export const voidPendingCashfreeOrder = async (req, res) => {
  try {
    ...
    if (order.paymentStatus !== PAYMENT_STATUS.PENDING) {
      // Already resolved (paid via webhook/poll race, already voided, etc.) — nothing to do.
      return res.status(200).json({ success: true, message: 'Already resolved' });
    }

    await handlePaymentFailed({
      order: { order_id: cashfreeOrderId },
      payment: { payment_message: 'Customer switched to a different payment method' },
    });

    return res.status(200).json({ success: true });
  } catch (error) {
    logger.error('Cashfree draft void error:', error);
    return res.status(500).json({
      success: false,
