...
export const getDashboard = async (req, res) => {
  try {
    ...
    const [totalOrders, salesResult, totalCustomers, recentOrders, lowStockProducts, ordersByStatus, todayResult, pendingCancelRequests] = await Promise.all([
      ...
      Product.find({ active: true, stock: { $lte: 10 } }).sort({ stock: 1 }).limit(10).select('name stock variants images').lean(),
...
export const getLowStockProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find({ active: true, stock: { $lte: threshold } }).sort({ stock: 1 }).populate('category', 'name slug').select('name slug stock variants images category').lean();
