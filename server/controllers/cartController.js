...
export const syncCart = async (req, res) => {
  try {
    ...
    for (const item of items) {
      if (!item.product || !item.variantId || !item.variantLabel || !item.name || item.price == null || item.quantity == null) {
        return res.status(400).json({
          success: false,
          message: 'Each item must have product, variantId, variantLabel, name, price, and quantity',
        });
      }
      if (item.quantity < 1) {
