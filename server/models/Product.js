...
const productSchema = new mongoose.Schema(
  {
    ...
    // Optional Tamil translations, filled in by hand from the admin panel.
    // Intentionally NOT required — every existing product has these as
    // empty strings until an admin adds a translation, and the storefront's
    // language toggle simply disables itself when descriptionTa is empty.
    // These are never sent to Seo.jsx / the prerender script's JSON-LD, so
    // they have zero effect on what Google indexes — English stays the only
    // crawlable version of this content (see the toggle's implementation in
    // ProductDetail.jsx for why that's a deliberate choice, not an oversight).
    descriptionTa: {
      type: String,
      default: '',
    },
    shortDescriptionTa: {
      type: String,
      default: '',
      maxlength: [200, 'Tamil short description cannot exceed 200 characters'],
    },
