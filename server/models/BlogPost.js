...
const blogPostSchema = new mongoose.Schema(
  {
    ...
    // Optional Tamil translation of the article body, filled in by hand from
    // the admin panel. Not required — existing posts default to an empty
    // string, and the blog's language toggle disables itself until an admin
    // adds one. Never used in Seo.jsx/meta tags or the prerendered JSON-LD
    // for this post, so it has no effect on what gets indexed — English
    // stays the only crawlable version, same reasoning as Product.descriptionTa.
    contentTa: {
      type: String,
      default: '',
    },
