...
const orderItemSchema = new mongoose.Schema({
  ...
  // Snapshot of which variant was bought and its label at time of order — kept even if
  // the admin later edits/removes that variant on the product, so order history always
  // correctly reflects what was actually purchased.
...
const orderSchema = new mongoose.Schema(
  {
    ...
    // Snapshot of the coupon actually applied at order time (name, code, percentage, and
    // the resulting rupee amount) - stored here rather than just referenced by code so
    // that a later edit to the coupon in Settings (or its removal) never rewrites the
    // history of what this order was actually discounted by.
    appliedCoupon: {
