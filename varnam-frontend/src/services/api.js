...
export const orderAPI = {
  ...
  // Guest (no login) self-service cancellation — order number + email OTP flow
  sendGuestCancelOtp: (orderNumber, email) => api.post('/orders/guest/cancel-otp/send', { orderNumber, email }),
  verifyGuestCancelOtp: (orderNumber, email, otp) => api.post('/orders/guest/cancel-otp/verify', { orderNumber, email, otp }),
  submitGuestCancelRequest: (orderNumber, email, reason, proofToken) =>
    api.post('/orders/guest/cancel-request', { orderNumber, email, reason, proofToken }),
  getGuestCancelRequestStatus: (orderNumber, email) =>
    api.get('/orders/guest/cancel-request/status', { params: { orderNumber, email } }),
...
export const settingsAPI = {
  ...
  validateCoupon: (code) => api.post('/settings/coupons/validate', { code }),
}
...
export const contactAPI = {
  ...
  getMessages: (status) => api.get('/contact', { params: status ? { status } : {} }),
...
export const uploadAPI = {
  ...
  uploadBlogImage: (file) => {
    const form = new FormData()
    form.append('image', file)
    return api.post('/upload/blog', form, { headers: { 'Content-Type': 'multipart/form-data' } })
  },
...
export const paymentAPI = {
  ...
  // Voids an abandoned Cashfree draft — called when the customer switches to a
  // different payment method after starting (and abandoning) an online payment.
  voidDraft: (cashfreeOrderId) => api.post('/payment/void-draft', { cashfreeOrderId }),
}
...
export const blogAPI = {
  ...
  getAdminById: (id) => api.get(`/blog/admin/${id}`),
