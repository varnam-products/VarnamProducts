...
import { orderAPI, paymentAPI, internationalOrderAPI, settingsAPI } from '../services/api'
import Seo from '../components/common/Seo'
import { decodeHtmlEntities } from '../utils/decodeHtmlEntities'
import { getVariantById, variantEffectivePrice } from '../utils/variants'
...
function effectivePrice(product, variantId) { return variantEffectivePrice(getVariantById(product, variantId)) }
...
function StateSelect({ value, onChange, error, onFocus, onBlur, dataErr }) {
  return (
    <div style={{ position: 'relative' }}>
      ...
      <span style={{
        ...
        color: '#A89F8C', pointerEvents: 'none', display: 'flex',
      }}>
        <IconChevronDown />
      </span>
    </div>
...
function OrderSuccessOverlay({ onDone, eyebrow = 'Order Confirmed', heading = 'Thank you! 🎉', subtext = "Your order is on its way.\nWe'll send updates to your email." }) {
  ...
  // Brand-toned palette so the burst still feels like Varnam, not a generic party popper
  const CONFETTI_COLORS = ['#2D6A4F', '#52B788', '#C8893A', '#E9B87A', '#95D5B2', '#FDF6EC']

  const fireConfetti = useCallback(() => {
    const defaults = { colors: CONFETTI_COLORS, disableForReducedMotion: true, zIndex: 400 }
    ...
    ;(function frame() {
      confetti({ ...defaults, particleCount: 4, angle: 60, spread: 55, startVelocity: 42, origin: { x: 0, y: 0.65 } })
      confetti({ ...defaults, particleCount: 4, angle: 120, spread: 55, startVelocity: 42, origin: { x: 1, y: 0.65 } })
  ...
  useGSAP(() => {
    ...
    tl.fromTo('.success-badge',
      { scale: 1 },
      { scale: 1.08, duration: 0.14, ease: 'power1.out', yoyo: true, repeat: 1 },
      '-=0.45'
    )
    tl.call(fireConfetti, null, '-=0.5')
  ...
  return (
    <div ref={overlayRef} style={{
      ...
      <div ref={cardRef} style={{
        ...
        <div className="success-badge" style={{ position: 'relative', width: 90, height: 90, margin: '0 auto 24px', zIndex: 5 }}>
          <div ref={pulseRef} style={{
            position: 'absolute', inset: 0, borderRadius: '50%',
            border: '2px solid #52B788', pointerEvents: 'none',
          }} />
...
function OrderSummary({
  items, subtotal, shippingFee, grandTotal, codEnabled, codLimit, international = false,
  couponInput, setCouponInput, couponChecking, appliedCoupon, couponError, discountAmount,
  onApplyCoupon, onRemoveCoupon, couponSectionRef,
}) {
  ...
  return (
    <div className="order-summary-card" style={{ background: '#fff', borderRadius: 18, border: '1px solid #F0EBE1', overflow: 'hidden', boxShadow: '0 4px 24px rgba(45,106,79,0.08)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '18px 22px', borderBottom: collapsed ? 'none' : '1px solid #F5F0E8' }}>
        <button
          ...
          <span style={{ color: '#A89F8C', transform: collapsed ? 'rotate(-90deg)' : 'rotate(0deg)', transition: 'transform 0.2s', marginLeft: 'auto' }}><IconChevronDown /></span>
        </button>
        <Link
          to="/cart"
          style={{
            ...
            color: '#2D6A4F', fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 700,
            textDecoration: 'none', transition: 'background 0.15s, transform 0.15s',
          }}
          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(45,106,79,0.16)'; e.currentTarget.style.transform = 'translateY(-1px)' }}
          onMouseLeave={e => { e.currentTarget.style.background = 'rgba(45,106,79,0.08)'; e.currentTarget.style.transform = 'translateY(0)' }}
        >
          <IconEdit /> Edit Cart
        </Link>
      </div>
        ...
        <>
          <div style={{ padding: '12px 22px 0', maxHeight: 240, overflowY: 'auto' }}>
            {items.map(({ product, variantId, quantity }) => {
              ...
              const price = effectivePrice(product, variantId)
              return (
                <div key={`${product._id}__${variantId}`} style={{ display: 'flex', gap: 12, alignItems: 'center', paddingBottom: 12, marginBottom: 12, borderBottom: '1px solid #F5F0E8' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: '#26221C', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{decodeHtmlEntities(product.name)}</p>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: '#A89F8C', margin: '2px 0 0' }}>
                      {variant?.label ? `${variant.label} · ` : ''}Qty: {quantity}
                    </p>
                  </div>
          ...
          {!international && (
            <div ref={couponSectionRef} style={{ margin: '0 22px 16px', padding: 12, borderRadius: 12, border: `1px solid ${appliedCoupon ? '#B7E4C7' : '#F0EBE1'}`, background: appliedCoupon ? '#F3FBF6' : '#FAFAF7' }}>
              {appliedCoupon ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span className="coupon-check-icon" style={{ width: 22, height: 22, borderRadius: '50%', background: '#52B788', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <IconCheck />
                  </span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: '#1B4332', margin: 0 }}>
                      {appliedCoupon.code} applied
                    </p>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: '#4B7A5C', margin: '2px 0 0' }}>
                      {appliedCoupon.name} · {appliedCoupon.discountPercentage}% off
                    </p>
                  </div>
                  <button onClick={onRemoveCoupon} type="button" aria-label="Remove coupon"
                    style={{ width: 24, height: 24, borderRadius: '50%', border: 'none', background: '#EAF6EE', color: '#4B7A5C', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                    <IconX />
                  </button>
                </div>
              ) : (
                <>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, background: '#fff', border: '1px solid #E8E2D6', borderRadius: 9, padding: '0 10px' }}>
                      <span style={{ color: '#A89F8C' }}><IconTag /></span>
                      <input
                        value={couponInput}
                        onChange={(e) => setCouponInput(e.target.value.toUpperCase())}
                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); onApplyCoupon() } }}
                        placeholder="Coupon code"
                        style={{ flex: 1, border: 'none', outline: 'none', padding: '9px 0', fontFamily: 'var(--font-body)', fontSize: 13, color: '#26221C', background: 'transparent', textTransform: 'uppercase' }}
                      />
                    </div>
                    <button
                      type="button"
                      ...
                      style={{
                        ...
                        background: (couponChecking || !couponInput.trim()) ? '#E8E2D6' : '#2D6A4F',
                        color: (couponChecking || !couponInput.trim()) ? '#A89F8C' : '#fff',
                        fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600,
                        cursor: (couponChecking || !couponInput.trim()) ? 'default' : 'pointer',
                        whiteSpace: 'nowrap',
                      }}>
                  ...
                  {couponError && (
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: '#B3261E', margin: '8px 2px 0' }}>{couponError}</p>
                  )}
                </>
              )}
            </div>
          ...
          <div style={{ padding: '12px 22px 20px' }}>
            ...
            {appliedCoupon && (
              <div className="coupon-discount-line" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, overflow: 'hidden' }}>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#2D6A4F', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <IconTag /> Coupon ({appliedCoupon.code})
                </span>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#2D6A4F', fontWeight: 600 }}>−{fmt(discountAmount)}</span>
              </div>
            )}
...
export default function Checkout() {
  ...
  // Coupon is entered fresh at checkout only — never persisted to the cart. It's gone
  // the moment the tab closes or the order goes through, by design.
  const [couponInput, setCouponInput]     = useState('')
  const [couponChecking, setCouponChecking] = useState(false)
  const [appliedCoupon, setAppliedCoupon] = useState(null) // { name, code, discountPercentage }
  ...
  // a second one). It changes whenever the cart contents change OR the payment method
  // changes, since either means the person is placing a genuinely different order attempt.
  // (Switching CASHFREE -> COD on an unchanged cart must NOT reuse the Cashfree draft's
  // key — the server's idempotency check would otherwise hand back that stale, still-
  // unpaid draft instead of actually creating the new COD order.)
  const idempotencyKeyRef = useRef(
    (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : `idem_${Date.now()}_${Math.random().toString(36).slice(2)}`
  )
  // The most recent Cashfree draft order's id, if one was created and hasn't been
  // resolved (paid/failed) yet — used to void it if the customer abandons it.
  const pendingCashfreeOrderIdRef = useRef(null)

  const itemsSignature = items.map(i => `${i.product._id}:${i.variantId}:${i.quantity}`).sort().join('|')
  const itemsSignatureRef = useRef(itemsSignature)
  const paymentMethodRef = useRef(paymentMethod)
  useEffect(() => {
    const itemsChanged  = itemsSignatureRef.current !== itemsSignature
    const methodChanged = paymentMethodRef.current !== paymentMethod

    if (itemsChanged || methodChanged) {
      // Abandoning an in-progress Cashfree attempt to submit a different order (new
      // cart contents, or a switch to COD) leaves its draft orphaned as "Pending
      // Payment" until its 24h TTL expiry unless we explicitly void it here.
      if (methodChanged && paymentMethodRef.current === 'CASHFREE' && pendingCashfreeOrderIdRef.current) {
        paymentAPI.voidDraft(pendingCashfreeOrderIdRef.current).catch(() => {
          // Best-effort — if this fails the draft simply expires naturally in 24h instead.
        })
        pendingCashfreeOrderIdRef.current = null
      }
  ...
  // Preview only — mirrors the server's math (discount applies to subtotal only, shipping
  // is already locked in above off the raw subtotal) so what's shown here matches what the
  // server will actually charge. The server recalculates all of this independently at order time.
  const discountAmount = appliedCoupon
    ? Number(((totals().subtotal * appliedCoupon.discountPercentage) / 100).toFixed(2))
    : 0
  const discountedGrandTotal = Number((totals().subtotal - discountAmount + totals().shippingFee).toFixed(2))

  useEffect(() => {
    if (paymentMethod === 'COD' && discountedGrandTotal > codLimit) {
      setPaymentMethod('CASHFREE')
    }
  }, [discountedGrandTotal, codLimit, paymentMethod])
  ...
  const handleApplyCoupon = async () => {
    ...
    setCouponError('')
    try {
      const { data } = await settingsAPI.validateCoupon(code)
      if (data.success) {
        setAppliedCoupon(data.data)
        setCouponInput(data.data.code)
      }
    } catch (err) {
      // Deliberately no animation here — a wrong code just shows the message, nothing more.
  ...
  // Only ever runs on a SUCCESSFUL apply — a bad code just sets couponError above and
  // never touches appliedCoupon, so this animation simply never fires for it.
  useGSAP(() => {
    if (!appliedCoupon || !couponSectionRef.current) return
    const el = couponSectionRef.current
    ...
    tl.fromTo(el,
      { boxShadow: '0 0 0 0 rgba(82,183,136,0.55)' },
      { boxShadow: '0 0 0 8px rgba(82,183,136,0)', duration: 0.7, ease: 'power2.out' },
      '<'
    )
    tl.fromTo(el.querySelector('.coupon-check-icon'),
      { scale: 0, rotate: -60, opacity: 0 },
      { scale: 1, rotate: 0, opacity: 1, duration: 0.5, ease: 'back.out(3)' },
      '<0.05'
    )
    tl.fromTo(el.querySelectorAll('.coupon-check-icon ~ div p'),
      { opacity: 0, x: -6 },
      { opacity: 1, x: 0, duration: 0.35, stagger: 0.06, ease: 'power2.out' },
      '<0.1'
    )
    const discountLine = document.querySelector('.coupon-discount-line')
    if (discountLine) {
      tl.fromTo(discountLine,
        { opacity: 0, y: -6 },
        { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' },
        '<0.1'
      )
    }
  }, { dependencies: [appliedCoupon?.code], scope: couponSectionRef })
  ...
  const handleCOD = async () => {
    ...
    try {
      const payload = {
        ...
        orderItems: items.map(({ product, variantId, quantity }) => ({ product: product._id, variantId, quantity })),
      ...
      toast.error(msg)
      // The code was valid when applied but got deactivated/removed before submit —
      // drop it from the UI so the total shown matches what the server actually charged.
  ...
  const handleCashfree = useCallback(async () => {
    ...
    try {
      ...
      cfOrder = data.data
      pendingCashfreeOrderIdRef.current = cfOrder.order_id
    } catch (err) {
  ...
  }, [form, items, t, clearCart, navigate, appliedCoupon])
  ...
  const handleIntlSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        items: items.map(({ product, variantId, quantity }) => {
          const variant = getVariantById(product, variantId)
          return {
            productId: product._id,
            name:      variant?.label ? `${product.name} (${variant.label})` : product.name,
            quantity,
            price:     effectivePrice(product, variantId),
          }
        }),
      }
  ...
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === 'COD') {
      ...
      if (discountedGrandTotal > codLimit) { toast.error(`COD is not available for orders above ${fmt(codLimit)}`); return }
      await handleCOD()
    } else {
  ...
  const codExceedsLimit = discountedGrandTotal > codLimit

  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF7', paddingBottom: 64 }}>
      <Seo title="Checkout" description="Complete your Varnam Foods order." path="/checkout" noindex />
                ...
                <button
                  ...
                  {submitting ? (
                    ...
                    <>
                      ...
                      {paymentMethod === 'CASHFREE' ? `Pay ${fmt(discountedGrandTotal)} Securely` : `Place Order — ${fmt(discountedGrandTotal)}`}
                      <IconArrowRight />
                    </>
                  )}
                </button>
                ...
                <SectionCard icon={<IconCard />} title="Selected Products" index={4}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {items.map(({ product, variantId, quantity }) => {
                      const variant = getVariantById(product, variantId)
                      return (
                        <div key={`${product._id}__${variantId}`} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 12px', borderRadius: 12, background: '#FAFAF7', border: '1px solid #F0EBE1' }}>
                          ...
                          <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: '#26221C', margin: 0, flexShrink: 0 }}>{fmt(effectivePrice(product, variantId) * quantity)}</p>
                        </div>
                      )
                    })}
                  </div>
