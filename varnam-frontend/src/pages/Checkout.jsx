...
function OrderSuccessOverlay({ onDone, eyebrow = 'Order Confirmed', heading = 'Thank you! 🎉', subtext = "Your order is on its way.\nWe'll send updates to your email." }) {
  ...
  const fireConfetti = useCallback(() => {
    ...
    // Big central pop, right as the checkmark lands
    confetti({
      ...defaults,
      particleCount: 90,
      spread: 100,
    ...
    // A second, softer pop a beat later for depth
    confetti({
      ...defaults,
      particleCount: 50,
      spread: 130,
    ...
    // Side cannons firing toward the centre, fireworks-style, for ~1.3s
  ...
  useGSAP(() => {
    ...
    // Glow ring expands outward from the checkmark right as it lands
    tl.fromTo(pulseRef.current,
      { scale: 0.6, opacity: 0.55 },
      { scale: 1.9, opacity: 0, duration: 0.75, ease: 'power2.out' },
      '-=0.4'
    )
    // A tiny bounce on the whole badge for extra punch
