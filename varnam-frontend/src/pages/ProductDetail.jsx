...
// Fields from server:  name, slug, shortDescription, description,
//   variants[{_id,label,price,discountPrice}], images[], stock, ingredients[],
//   benefits[], category{name,slug}, featured, bestSeller, ratings, totalSales
...
import { getVariantById, getCheapestVariant, variantEffectivePrice, variantDiscountPct } from '../utils/variants'
...
export default function ProductDetail() {
  ...
  // English/Tamil display toggle — purely client-side, resets to English on
  // every new product so switching products never leaves a stale language
  // selected for content that may not have a Tamil translation.
  ...
  useEffect(() => {
    ...
    productAPI.getBySlug(slug)
      .then(({ data }) => {
        ...
        // Default to the cheapest variant on load — the customer can switch from there.
        setSelectedVariantId(getCheapestVariant(data.data)?._id ?? null)
        setLoading(false)
      })
      .catch(err => {
  ...
  const handleAddToCart = () => {
    if (!product || product.stock === 0 || !selectedVariantId) return
    addItem(product, selectedVariantId, qty)
    // Bounce the badge
    if (badgeRef.current) {
  ...
  const {
    _id, name: rawName, shortDescription: rawShortDescription, description: rawDescription,
    shortDescriptionTa: rawShortDescriptionTa, descriptionTa: rawDescriptionTa, variants,
  ...
  const shortDescriptionTa = decodeHtmlEntities(rawShortDescriptionTa)
  const descriptionTa      = decodeHtmlEntities(rawDescriptionTa)

  // The toggle is only enabled when a Tamil translation of the main
  // description actually exists — the short description translating too is
  // a bonus, not a requirement, since the full description is the content
  // that matters most to a reader deciding whether to buy.
  const hasTamil = Boolean(descriptionTa?.trim())
  const displayShortDescription = (lang === 'ta' && shortDescriptionTa?.trim()) ? shortDescriptionTa : shortDescription
  const displayDescription      = (lang === 'ta' && hasTamil) ? descriptionTa : description

  // The currently-selected variant drives price/discount/add-to-cart — falls back to
  // the cheapest variant if selection hasn't resolved yet (e.g. first render).
  const selectedVariant = getVariantById(product, selectedVariantId) || getCheapestVariant(product)
  const price           = selectedVariant?.price ?? 0
  const effectivePrice  = variantEffectivePrice(selectedVariant)
  const discount        = variantDiscountPct(selectedVariant)
  const outOfStock      = stock === 0
  const lowStock        = !outOfStock && stock <= 5
  const inCart          = selectedVariant ? isInCart(_id, selectedVariant._id) : false
  const cartQty         = selectedVariant ? getItemQty(_id, selectedVariant._id) : 0
      ...
      <div className="container-main py-8 lg:py-12">
        <div className="grid lg:grid-cols-2 gap-10 xl:gap-16 items-start">
          ...
          <div ref={infoRef} className="flex flex-col gap-0">
            ...
            {/* Short description */}
            <div className="info-row flex items-start justify-between gap-3 mb-5" style={{ opacity: 0 }}>
              <p className={`${lang === 'ta' && shortDescriptionTa?.trim() ? 'font-tamil' : 'font-body'} text-sm text-neutral-500 leading-relaxed flex-1`}>
                {displayShortDescription}
              </p>
              <LangToggle lang={lang} onChange={setLang} available={hasTamil} />
            </div>

            {/* Variant selector — only renders when the product has more than one variant */}
            <VariantSelector
              variants={variants}
            ...
            <div className="info-row border border-neutral-100 rounded-2xl overflow-hidden divide-y divide-neutral-100"
              ...
              <div className="px-5">
                <AccordionTab title="Description" icon={LeafIcon} defaultOpen>
                  <p className={`whitespace-pre-line ${lang === 'ta' && hasTamil ? 'font-tamil' : ''}`}>{displayDescription}</p>
                </AccordionTab>
              </div>
