...
const IconVideo   = ({ size = 18 }) => <Ico size={size} d={<><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></>} />
const IconInfo    = () => <Ico size={14} d={<><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></>} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {banner.image && (
          banner.mediaType === 'video'
            ? <video src={banner.image} muted autoPlay loop playsInline style={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 10, border: '1px solid #F0EBE1' }} />
...
function BannerFormModal({ initial, onSave, onClose }) {
  ...
  const handleImageFile = async (file) => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, image: url, publicId: pid, videoDuration: null }))
  ...
  // Reads video duration client-side (before uploading anything) so we can
  // reject clips over 10s instantly, without wasting the admin's data or
  // waiting on a round trip to Cloudinary.
  ...
  const handleVideoFile = async (file) => {
    ...
    if (file.size > MAX_VIDEO_MB * 1024 * 1024) {
      toast.error(`Video is too large — keep it under ${MAX_VIDEO_MB}MB.`)
      return
    }
    ...
    try {
      ...
      if (duration > MAX_VIDEO_SECONDS + 0.5) {
        toast.error(`Video is ${duration.toFixed(1)}s — please trim it to ${MAX_VIDEO_SECONDS}s or under.`)
        return
      }
      setUploading(true)
      const { data } = await uploadAPI.uploadBannerVideo(file)
      const url = data.url || data.data?.url || ''
      const pid = data.publicId || data.data?.publicId || ''
      setForm(prev => ({ ...prev, image: url, publicId: pid, videoDuration: data.duration ?? duration }))
  ...
  const handleFile = (file) => {
    if (!file) return
    if (form.mediaType === 'video') handleVideoFile(file)
  ...
  const handleSubmit = async () => {
    if (!form.image) { toast.error(`Please upload a banner ${form.mediaType}`); return }
    setSaving(true)
    try {
      await onSave(form)
    } finally {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  if (form.mediaType === key) return
                  setForm(prev => ({ ...prev, mediaType: key, image: '', publicId: '', videoDuration: null }))
                }}
                style={{
                  flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
                  padding: '9px 12px', borderRadius: 10, cursor: 'pointer',
                  fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600,
                  border: `1.5px solid ${form.mediaType === key ? '#2D6A4F' : '#E8E0D0'}`,
                  background: form.mediaType === key ? 'rgba(45,106,79,0.08)' : '#fff',
                  color: form.mediaType === key ? '#2D6A4F' : '#7A7265',
        ...
        <div>
          <label style={labelStyle}>
            Banner {form.mediaType === 'video' ? 'Video' : 'Image'} <span style={{ color: '#DC2626' }}>*</span>
          </label>

          {form.mediaType === 'video' && (
            <div style={{ display: 'flex', gap: 8, background: 'rgba(45,106,79,0.06)', border: '1px solid rgba(45,106,79,0.15)', borderRadius: 10, padding: '9px 12px', marginBottom: 10 }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: '#2D6A4F', margin: 0, lineHeight: 1.55 }}>
                Free Cloudinary plan — keep clips <strong>10 seconds or under</strong> and <strong>under 20MB</strong>.
                For best quality, upload landscape video around <strong>1920×1080px</strong> (or the same widescreen
                ratio as your banner slot). Longer clips get auto-trimmed to the first 10 seconds.
              </p>
            </div>
          )}
          ...
          >
            {form.image ? (
              <div style={{ position: 'relative', width: '100%' }}>
                ...
                {form.mediaType === 'video' && form.videoDuration != null && (
                  <span style={{ position: 'absolute', bottom: 8, left: 8, background: 'rgba(38,34,28,0.75)', color: '#fff', fontFamily: 'var(--font-body)', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 6 }}>
                    {Number(form.videoDuration).toFixed(1)}s
                  </span>
                )}
                <button
                  onClick={e => { e.stopPropagation(); setForm(prev => ({ ...prev, image: '', publicId: '', videoDuration: null })) }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '24px 0', color: '#A89F8C' }}>
                {form.mediaType === 'video' ? <IconVideo size={26} /> : <IconUpload />}
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, margin: 0 }}>Click or drag to upload</p>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, margin: 0, color: '#C8BDB0' }}>
                  {form.mediaType === 'video' ? 'MP4/WebM/MOV · max 10s · under 20MB' : 'Recommended: 1440×600px'}
                </p>
              </div>
            )}
          </div>
    ...
    <div ref={rowRef} className="banner-row" style={{
      ...
      <div className="banner-row-inner">
        ...
        <div className="banner-thumb">
          {banner.image
            ? banner.mediaType === 'video'
              ? <video src={banner.image} muted loop autoPlay playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', minHeight: 88 }} />
          ...
          {banner.mediaType === 'video' && (
            <div style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(45,106,79,0.85)', color: '#fff', display: 'flex', alignItems: 'center', gap: 3, fontFamily: 'var(--font-body)', fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 5 }}>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (deleteTarget.publicId) {
        try { await uploadAPI.deleteAsset(deleteTarget.publicId, deleteTarget.mediaType === 'video' ? 'video' : 'image') } catch { /* non-fatal */ }
      }
