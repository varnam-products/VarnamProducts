              ...
              {order.orderItems.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: '#FAFAF7', borderRadius: 11, border: '1px solid #F0EBE1' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: '#26221C', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {decodeHtmlEntities(item.name)}
                      {item.variantLabel && <span style={{ fontWeight: 400, color: '#A89F8C' }}> · {item.variantLabel}</span>}
                    </p>
