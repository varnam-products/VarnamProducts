...
const IconTag       = ({ size = 16 }) => <Ico size={size} d={<><path d="M20.59 13.41 11 3.83A2 2 0 0 0 9.59 3.24H4a1 1 0 0 0-1 1v5.59a2 2 0 0 0 .59 1.41l9.58 9.59a2 2 0 0 0 2.83 0l5.59-5.59a2 2 0 0 0 0-2.83z"/><circle cx="7.5" cy="7.5" r="1.3" fill="currentColor" stroke="none"/></>} />
const IconPlus      = ({ size = 15 }) => <Ico size={size} d={<><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></>} />
const IconTrash     = ({ size = 15 }) => <Ico size={size} d={<><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></>} />
...
function PercentInput({ value, onChange, disabled }) {
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
      <input
        type="number" min={1} max={100} value={value} disabled={disabled}
        onChange={e => onChange(e.target.value === '' ? '' : Number(e.target.value))}
        style={{
          width: '100%', boxSizing: 'border-box',
          padding: '10px 28px 10px 12px', borderRadius: 10,
          border: '1.5px solid #E8E0D0', background: disabled ? '#F5F0E8' : '#FAFAF7',
          fontFamily: 'var(--font-body)', fontSize: 13, color: '#26221C',
          outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s',
        }}
...
export default function AdminSettings() {
      ...
      try {
        ...
        if (data.success) {
          ...
          const loaded = {
            ...
            coupons: Array.isArray(s.coupons) ? s.coupons.map(c => ({
              name: c.name || '',
              code: c.code || '',
  ...
  // ── Coupon list helpers ──────────────────────────────────────────────────────
  const addCoupon = () => {
    if (form.coupons.length >= 3) return
    setForm(prev => ({
      ...prev,
      coupons: [...prev.coupons, { name: '', code: '', discountPercentage: 10, active: true }],
    }))
  }
  ...
  const updateCoupon = (index, key) => (value) => {
    setForm(prev => ({
      ...prev,
      coupons: prev.coupons.map((c, i) => (i === index ? { ...c, [key]: value } : c)),
    }))
    setErrors(prev => ({ ...prev, [`coupon_${index}_${key}`]: undefined }))
  }
  ...
  const validate = () => {
    ...
    form.coupons.forEach((c, i) => {
      const name = c.name?.trim()
      const code = c.code?.trim().toUpperCase()
      if (!name) e[`coupon_${i}_name`] = 'Name is required'
      if (!code) e[`coupon_${i}_code`] = 'Code is required'
      if (c.discountPercentage === '' || c.discountPercentage < 1 || c.discountPercentage > 100)
        e[`coupon_${i}_discountPercentage`] = 'Must be between 1 and 100'
      if (code) {
        if (seenCodes.has(code)) {
          e[`coupon_${i}_code`] = 'Duplicate code'
          e[`coupon_${seenCodes.get(code)}_code`] = 'Duplicate code'
        }
        ...
        {loading ? (
          ...
          <div ref={bodyRef} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <Section
              title="Coupon Codes"
              icon={<IconTag />}
              hint="Up to 3 active codes. Entered by customers at checkout only — never tied to a saved cart."
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {form.coupons.length === 0 && (
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: '#A89F8C', margin: 0 }}>
                    No coupon codes yet.
                  </p>
                )}
                ...
                {form.coupons.map((coupon, i) => (
                  <div key={i} style={{
                    border: '1.5px solid #E8E0D0', borderRadius: 12, padding: 16,
                    background: coupon.active ? '#FAFAF7' : '#F5F0E8',
                  }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                      <Field label="Name" error={errors[`coupon_${i}_name`]}>
                        <TextInput
                          value={coupon.name}
                          onChange={updateCoupon(i, 'name')}
                          placeholder="Diwali Sale"
                        />
                      </Field>
                      <Field label="Code" error={errors[`coupon_${i}_code`]}>
                        <TextInput
                          value={coupon.code}
                          onChange={(v) => updateCoupon(i, 'code')(v.toUpperCase())}
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
                      <div style={{ width: 140 }}>
                        <Field label="Discount" error={errors[`coupon_${i}_discountPercentage`]}>
                          <PercentInput
                            value={coupon.discountPercentage}
                            onChange={updateCoupon(i, 'discountPercentage')}
                          />
                        </Field>
                      </div>
                      ...
                      <div style={{ flex: 1, paddingBottom: 10 }}>
                        <Toggle
                          ...
                          onChange={updateCoupon(i, 'active')}
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: '1.5px solid #E8E0D0', background: '#fff', color: '#B3261E',
                ...
                {form.coupons.length < 3 ? (
                  <button
                    ...
                    style={{
                      ...
                      border: '1.5px dashed #B7CFC2', background: 'rgba(45,106,79,0.04)', color: '#2D6A4F',
                  ...
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#A89F8C', margin: 0 }}>
                    Maximum of 3 coupon codes reached.
                  </p>
                )}
              </div>
            </Section>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <Field label="Store Name">
                  <TextInput value={form.storeName} onChange={set('storeName')} placeholder="Varnam Foods" />
                </Field>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
