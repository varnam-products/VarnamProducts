// src/pages/admin/AdminB2BInquiries.jsx
// API:
//   GET   /api/b2b/inquiries            → { success, data: [] }   (optional ?status= filter)
//   PATCH /api/b2b/inquiries/:id/status → { success, data: {} }   (status: New | Contacted | Closed | Cancelled)
...
const initials = (name = '') =>
  name.trim().split(' ').slice(0, 2).map(w => w[0]?.toUpperCase()).join('') || 'B'
...
const IcoBuilding = () => <Ico><rect x="4" y="2" width="16" height="20" rx="1"/><line x1="9" y1="7" x2="9" y2="7.01"/><line x1="15" y1="7" x2="15" y2="7.01"/><line x1="9" y1="11" x2="9" y2="11.01"/><line x1="15" y1="11" x2="15" y2="11.01"/><line x1="9" y1="15" x2="9" y2="15.01"/><line x1="15" y1="15" x2="15" y2="15.01"/></Ico>
...
/* ─── inquiry row ─────────────────────────────────────────────────── */
function InquiryRow({ inquiry, onView }) {
  return (
    <tr
      ...
      <td style={{ padding: '13px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(45,106,79,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 13, color: '#2D6A4F', fontWeight: 700 }}>{initials(inquiry.businessName)}</span>
          </div>
          <div style={{ minWidth: 0 }}>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: '#26221C', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 190 }}>{inquiry.businessName}</p>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: '#A89F8C', margin: '2px 0 0' }}>{inquiry.businessType}</p>
          </div>
        </div>
      </td>
      <td style={{ padding: '13px 14px' }}>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#3D3830', margin: 0 }}>{inquiry.contactName}</p>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: '#A89F8C', margin: '2px 0 0' }}>{inquiry.phone}</p>
      </td>
      <td style={{ padding: '13px 14px', fontFamily: 'var(--font-body)', fontSize: 13, color: '#5C5548' }}>{inquiry.city || '—'}</td>
      <td style={{ padding: '13px 14px', fontFamily: 'var(--font-body)', fontSize: 13, color: '#5C5548' }}>{inquiry.monthlyQuantity || '—'}</td>
      <td style={{ padding: '13px 14px', fontFamily: 'var(--font-body)', fontSize: 12.5, color: '#A89F8C', whiteSpace: 'nowrap' }}>{fmtDate(inquiry.createdAt)}</td>
      <td style={{ padding: '13px 14px' }}><StatusBadge status={inquiry.status} /></td>
    </tr>
  )
}

/* ─── detail drawer ───────────────────────────────────────────────── */
function InquiryDetailModal({ inquiry: initial, onClose, onUpdated }) {
  ...
  const handleStatusChange = async (status) => {
    ...
    setUpdating(true)
    try {
      const { data } = await b2bAPI.updateInquiryStatus(inquiry._id, status)
      if (data.success) {
    ...
    <div ref={overlayRef} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(20,18,14,0.35)', backdropFilter: 'blur(2px)' }} onClick={close}>
      <div ref={panelRef} onClick={e => e.stopPropagation()}
        ...
        <div style={{ padding: '20px 22px', borderBottom: '1px solid #F0EBE1', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 18, color: '#26221C', margin: '0 0 4px' }}>{inquiry.businessName}</h2>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 22px 22px' }}>
          <Row Icon={IcoBuilding} label="Business type" value={inquiry.businessType} />
          <Row Icon={IcoUser}     label="Contact person" value={inquiry.contactName} />
          <Row Icon={IcoPhone}    label="Phone" value={inquiry.phone} href={`tel:${inquiry.phone}`} />
          {inquiry.email && <Row Icon={IcoMail} label="Email" value={inquiry.email} href={`mailto:${inquiry.email}`} />}
          {inquiry.city && <Row Icon={IcoMapPin} label="City" value={inquiry.city} />}
          {inquiry.monthlyQuantity && <Row Icon={IcoBox} label="Estimated monthly quantity" value={inquiry.monthlyQuantity} />}

          {inquiry.products?.length > 0 && (
            <div style={{ marginTop: 6, paddingTop: 14, borderTop: '1px solid #F5F0E8' }}>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 10.5, color: '#A89F8C', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 8px' }}>Products of interest</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {inquiry.products.map((p, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 10, background: '#FAFAF7', border: '1px solid #F0EBE1' }}>
                    ...
                    {p.quantity && <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#A89F8C' }}>Qty: {p.quantity}</span>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {inquiry.message && (
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #F5F0E8' }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color: '#3D3830', lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap' }}>{inquiry.message}</p>
            </div>
          )}
...
export default function AdminB2BInquiries() {
  ...
  const fetchInquiries = useCallback(async (status = tab) => {
    setLoading(true)
    try {
      const { data } = await b2bAPI.getInquiries(status === 'All' ? undefined : status)
  ...
  useEffect(() => { fetchInquiries(tab) }, [tab])
  ...
  const handleUpdated = (updated) => {
    setInquiries(prev => {
      ...
      if (tab !== 'All' && updated.status !== tab) return prev.filter(i => i._id !== updated._id)
      return prev.map(i => i._id === updated._id ? updated : i)
    })
    setViewInquiry(updated)
  }

  const counts = useMemo(() => {
    const c = { New: 0, Contacted: 0, Closed: 0, Cancelled: 0 }
    inquiries.forEach(i => { if (c[i.status] !== undefined) c[i.status]++ })
    return c
  }, [inquiries])
  ...
  const visible = search
    ? inquiries.filter(i => {
        ...
        return i.businessName.toLowerCase().includes(q) ||
  ...
  return (
    <div>
      <div ref={headerRef} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 22 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.3rem,2vw,1.7rem)', color: '#26221C', margin: '0 0 4px' }}>B2B Inquiries</h1>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#A89F8C', margin: 0 }}>
            {loading ? 'Loading…' : `${inquiries.length} inquiries · ${counts.New} new`}
          </p>
        </div>
        <button onClick={() => fetchInquiries(tab)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderBottom: '1px solid #F5F0E8', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 10, border: '1.5px solid #E8E0D0', background: '#FAFAF7', flex: 1, minWidth: 220 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search business, contact, phone, email or city…"
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: '#FAFAF7', borderBottom: '1px solid #F0EBE1' }}>
                {['Business', 'Contact', 'City', 'Monthly Qty', 'Submitted', 'Status'].map((h, i) => (
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '48px 14px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: '#C0B8A5' }}>
                      <IcoInbox />
                      <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#A89F8C' }}>No inquiries found</span>
                    </div>
                  </td>
                </tr>
              ) : (
                visible.map(i => <InquiryRow key={i._id} inquiry={i} onView={setViewInquiry} />)
              )}
            </tbody>
          </table>
        </div>
