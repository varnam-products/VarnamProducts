...
function CategoryFormModal({ initial, onSave, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    if (description.trim().length > 1000) { toast.error('Description cannot exceed 1000 characters'); return }
    setSaving(true)
    try {
      await onSave({ name: name.trim(), description: description.trim(), image: imageUrl || undefined })
    } finally {
      setSaving(false)
    }
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Shown on the category page and used as SEO/meta description. Leave blank to use the default templated sentence."
            rows={4}
            maxLength={1000}
            ...
            onBlur={e  => { e.target.style.borderColor = '#E8E0D0'; e.target.style.boxShadow = 'none' }}
          />
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: description.length > 1000 ? '#DC2626' : '#A89F8C', margin: '5px 0 0' }}>
            {description.length}/1000 — aim for ~155 characters if this is mainly for the meta description snippet.
          </p>
        </div>
