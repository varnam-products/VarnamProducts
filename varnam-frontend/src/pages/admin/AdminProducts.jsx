        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Field label="Short Description (Tamil)" hint={`${form.shortDescriptionTa.length}/200 chars — optional, shown when a customer taps "தமிழ்" on the product page`}>
            <textarea value={form.shortDescriptionTa} onChange={e => set('shortDescriptionTa', e.target.value)}
              maxLength={200} placeholder="குறுகிய தமிழ் விளக்கம் (விருப்பத்தேர்வு)…" style={{ ...textareaCls(), minHeight: 64 }}
          ...
          <Field label="Full Description (Tamil)" hint='Optional — leave blank to keep the "தமிழ்" toggle disabled for this product'>
            <textarea value={form.descriptionTa} onChange={e => set('descriptionTa', e.target.value)}
              placeholder="முழு தமிழ் விளக்கம் (விருப்பத்தேர்வு)…" style={textareaCls()}
              onFocus={e => e.target.style.borderColor = '#2D6A4F'} onBlur={e => e.target.style.borderColor = '#E8E0D0'} />
          </Field>
