...
const EMPTY_FORM = {
  ...
  descriptionTa: '', shortDescriptionTa: '',
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        shortDescriptionTa: form.shortDescriptionTa.trim(),
    ...
    <div ref={overlayRef} style={{
      ...
      <div ref={drawerRef} style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* Tamil translations — entirely optional. Leaving these blank just means the
              storefront's EN/தமிழ் toggle stays disabled for this product; nothing else
              breaks, and these are never sent to Seo.jsx or used for SEO meta/JSON-LD.
              fontFamily is overridden to var(--font-tamil) here (Catamaran) since DM Sans
              has no Tamil glyphs — without this the textarea falls back to whatever font
              the OS picks, which won't match what the storefront actually renders. */}
          <Field label="Short Description (Tamil)" hint={`${form.shortDescriptionTa.length}/200 chars — optional, shown when a customer taps "தமிழ்" on the product page`}>
            <textarea value={form.shortDescriptionTa} onChange={e => set('shortDescriptionTa', e.target.value)}
              maxLength={200} placeholder="குறுகிய தமிழ் விளக்கம் (விருப்பத்தேர்வு)…" style={{ ...textareaCls(), minHeight: 64, fontFamily: 'var(--font-tamil)' }}
              onFocus={e => e.target.style.borderColor = '#2D6A4F'} onBlur={e => e.target.style.borderColor = '#E8E0D0'} />
          </Field>

          <Field label="Full Description (Tamil)" hint='Optional — leave blank to keep the "தமிழ்" toggle disabled for this product'>
            <textarea value={form.descriptionTa} onChange={e => set('descriptionTa', e.target.value)}
              placeholder="முழு தமிழ் விளக்கம் (விருப்பத்தேர்வு)…" style={{ ...textareaCls(), fontFamily: 'var(--font-tamil)' }}
              onFocus={e => e.target.style.borderColor = '#2D6A4F'} onBlur={e => e.target.style.borderColor = '#E8E0D0'} />
          </Field>
