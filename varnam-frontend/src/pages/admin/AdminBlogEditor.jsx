...
const emptyForm = {
  title: '', excerpt: '', content: '', contentTa: '', coverImage: '', coverImagePublicId: '',
    ...
    blogAPI.getAdminById(id)
      .then(({ data }) => {
        ...
        setForm({
          ...
          contentTa: decodeHtmlEntities(p.contentTa) || '',
...
Leave a blank line between paragraphs — that's how the site tells them apart."
          ...
          {/* Tamil translation — entirely optional. Leaving this blank just means the
              blog's EN/தமிழ் toggle stays disabled for this post; nothing else breaks,
              and this is never sent to Seo.jsx or used for SEO meta/JSON-LD. */}
          <div style={cardStyle}>
            <label style={labelStyle}>
              <span>Content (Tamil) <span style={{ fontWeight: 400, color: '#A89F8C' }}>— optional, shown when a reader taps "தமிழ்" on the article page</span></span>
            </label>
            <textarea value={form.contentTa} onChange={set('contentTa')} placeholder="இங்கே தமிழில் கட்டுரையை எழுதவும் (விருப்பத்தேர்வு)…

பத்திகளுக்கு இடையே ஒரு வெற்று வரியை விடவும்."
              rows={18} style={{ ...inputStyle, resize: 'vertical', fontFamily: 'var(--font-tamil)', lineHeight: 1.7, minHeight: 360 }} onFocus={focus} onBlur={blur} />
          </div>
