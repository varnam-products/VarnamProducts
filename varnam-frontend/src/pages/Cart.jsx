...
function CartItem({ item, onRemove, onQtyChange }) {
  ...
  const hasDisc   = variant?.discountPrice > 0
  ...
  const handleRemove = () => {
    gsap.to(rowRef.current, {
      ...
      onComplete: () => onRemove(product._id, variantId),
    })
  }
  ...
  const handleQty = (delta) => {
    ...
    onQtyChange(product._id, variantId, next)
  }
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {variant?.label && (
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: '#A89F8C', margin: '3px 0 0' }}>
                {variant.label}
              </p>
            )}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            {hasDisc && (
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: '#52B788', margin: 0 }}>was {fmt(variant.price * quantity)}</p>
            )}
          </div>
        </div>
...
function OrderSummary({ items, totals, settings, onCheckout, isAuthenticated }) {
  ...
  const totalSavings = items.reduce((acc, { product, variantId, quantity }) => {
    const variant = getVariantById(product, variantId)
    if (variant?.discountPrice > 0) acc += (variant.price - variant.discountPrice) * quantity
    return acc
  }, 0)
            ...
            <div className="cart-layout">
              <div>
                <div ref={listRef} style={{ background: '#fff', borderRadius: 20, border: '1px solid #F0EBE1', padding: '0 24px', boxShadow: '0 2px 16px rgba(45,106,79,0.06)' }}>
                  {items.map(item => (
                    <div key={`${item.product._id}__${item.variantId}`} className="cart-row">
