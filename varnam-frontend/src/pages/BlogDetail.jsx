...
function ArticleBody({ content, tamil = false }) {
  const decoded = decodeHtmlEntities(content) || ''
  const paragraphs = decoded.split(/\n\s*\n/).map((p) => p.trim()).filter(Boolean)

  return (
    <div className={`${tamil ? 'font-tamil' : 'font-body'} text-[15.5px] sm:text-[16.5px] leading-[1.85] text-neutral-700 space-y-5`}>
...
export default function BlogDetail() {
  ...
  // English/Tamil display toggle for the article body — purely client-side,
  // resets to English on every new post so switching articles never leaves
  // Tamil selected for a post that may not have a translation.
  ...
  // Tamil translation of the article body only — never used for SEO title/
  // excerpt/JSON-LD above, so this has no effect on what Google indexes.
  const hasTamil = Boolean(post.contentTa?.trim())
  const displayContent = (lang === 'ta' && hasTamil) ? post.contentTa : post.content
      ...
      <div className="container-main max-w-4xl pb-16">
        <div className="flex flex-col sm:flex-row gap-10">
          ...
          <article className="flex-1 min-w-0">
            <ArticleBody content={displayContent} tamil={lang === 'ta' && hasTamil} />
