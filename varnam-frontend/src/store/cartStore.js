...
/**
 * A cart line's real identity is product + variant — the same product in
 * two different variants (e.g. 500ml vs 1L) must sit as two separate lines.
 */
function itemKey(item) {
  return `${item.product._id}__${item.variantId}`
}

function effectivePrice(product, variantId) {
  return variantEffectivePrice(getVariantById(product, variantId))
}

function computeTotals(items, freeShippingThreshold, flatShippingFee) {
  const subtotal = items.reduce(
    (sum, { product, variantId, quantity }) => sum + effectivePrice(product, variantId) * quantity,
    0
  )
 ...
 *   { product: id, variantId, variantLabel, name, price, image, quantity }
 *
 * Uses effectivePrice so the persisted price always reflects the actual
 * charged amount (discountPrice when set, otherwise price) for the
 * selected variant.
 */
function toServerItems(items) {
  return items.map(({ product, variantId, quantity }) => {
...
export const useCartStore = create(
  persist(
    (set, get) => ({
       ...
       *        - For each server item whose product+variant is NOT already in local,
       *          skip it — we can't reconstruct the full product object from the flat
       *          server shape, and showing a half-hydrated item would break the UI.
       *        - For each server item whose product+variant IS in local, take the
      ...
      hydrateFromDB: async () => {
        try {
          ...
          // Index local items by product+variant ID for O(1) lookup
          const localByKey = {}
          for (const li of localItems) {
            localByKey[itemKey(li)] = li
          }
          const serverKey = (si) => `${si.product}__${si.variantId}`

          // Start with local items, bumping quantity where server had more
          const mergedItems = localItems.map((localItem) => {
            const si = serverItems.find((s) => serverKey(s) === itemKey(localItem))
          ...
          // Reconstruct a minimal product object (with a single-variant
          // snapshot) from the flat server shape — enough for the cart UI
          // to render name, price, image, and quantity.
          // Fields the server doesn't return (stock, slug, etc.) are given
          // safe defaults so nothing in the UI breaks.
          for (const si of serverItems) {
            if (localByKey[serverKey(si)]) continue  // already handled above
            mergedItems.push({
              product: {
                _id:   si.product,
                name:  si.name,
                variants: [{
                  _id:           si.variantId,
                  label:         si.variantLabel,
                  price:         si.price,   // flat server price is already effective
                  discountPrice: 0,
                }],
      ...
      addItem: (product, variantId, quantity = 1) => {
        ...
        if (existingIndex !== -1) {
          ...
          updatedItems = [...items, { product, variantId, quantity: safeQty }]
        }
      ...
      isInCart: (productId, variantId) =>
        get().items.some((item) => item.product._id === productId && item.variantId === variantId),

      getItemQty: (productId, variantId) => {
        const item = get().items.find((i) => i.product._id === productId && i.variantId === variantId)
